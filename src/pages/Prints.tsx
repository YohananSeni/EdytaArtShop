...
const Prints: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPrints = async () => {
      try {
        setLoading(true);
        const data = await getProductsByCategory('print');
        setPrints(data);
        setLoading(false);
      } catch (error) {
  ...
  const sortedPrints = [...prints].sort((a: any, b: any) => {
    switch (sortBy) {
      case 'price-low':
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-neutral-600 max-w-xl mx-auto">
            Discover our collection of high-quality art prints. Each piece is printed 
            with archival inks on premium paper for vibrant colors and lasting beauty.
          </p>
        </motion.div>
        ...
        {/* Products Grid */}
        <ProductGrid products={sortedPrints} loading={loading} />
      </div>
    </div>
  );
};
